  ...
  return (
    <div className="relative group">
        <div 
            ...
            // Pause on hover by clearing interval logic (handled implicitly by user interaction often, but explicitly requires state if strictly needed. Here simplified)
        >
             {reviews.map(review => (
        ...
        {/* Navigation Buttons (Visible on hover or always visible on mobile? Let's make them subtle) */}
        <button 
            onClick={() => handleScroll('left')}
            className="absolute left-0 top-1/2 -translate-y-1/2 bg-white/80 p-2 rounded-full shadow-md hover:bg-white hidden sm:flex"
        >
            <ChevronLeftIcon size={20} />
        </button>
        <button 
            onClick={() => handleScroll('right')}
            className="absolute right-0 top-1/2 -translate-y-1/2 bg-white/80 p-2 rounded-full shadow-md hover:bg-white hidden sm:flex"
        >
